---
name: ahe-fix
description: Create a concrete `.plans/{plan_name}.md` fix plan for errors, bugs, or user-intent changes. Use when the user explicitly invokes `$ahe-fix`, `ahe-fix`, or `ahe fix`, and call `ahe-conversator` when the plan needs clarification.
---

# AHE Fix

`ahe-fix` is a user-facing AHE command for creating a fix plan. It is separate
...
3. If the fix goal, scope, or success criteria are unclear, call
   `ahe-conversator` and ask one focused question before writing the plan.
...
- If the requested fix is ambiguous, use `ahe-conversator` instead of guessing.
...
```bash
python3 .codex/skills/ahe-fix/scripts/write_fix_plan.py \
  --root "$PWD" \
